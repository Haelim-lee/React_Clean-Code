    ...
    const uploadImage =()=> {
      ...
      return (
        <div>
          ...
          {Session.State != 'LOGIN' ?
          <h1 style={{alignSelf:'center'}}>로그인이 필요합니다.</h1>:
          ...
          {Feed.length === 0 ? '':Feed.map((Data,i) => (
                <img width="180" height="180"  src={Data.pix_loadPath!=undefined?'http://antsnest.co.kr:4000/ftp/DeveloperFile/'+Data.pix_loadPath:'http://antsnest.co.kr:4000/ftp/FileUpload/'+'NoImage.png'}/>
            ))}
            </div>
      </div>
      );
