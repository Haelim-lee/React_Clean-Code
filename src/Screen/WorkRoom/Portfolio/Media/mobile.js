...
function Media(props) {
  ...
  const CreateSession=()=>{
    ...
    .then(res => res.json())
    .then((msg) => {
      if(msg.State ==='LOGIN') GetMediaPhoto_ORG();
      setSession(msg)})
  ...
  const uploadImgPreview=()=>{
    ...
    const GetMediaPhoto_ORG=()=>{
      fetch('http://antsnest.co.kr:4000/GetMediaPhoto_ORG',{credentials:'include'})
      .then(res => res.json())
    ...
    const uploadImage =()=> {
      ...
      fetch('http://antsnest.co.kr:4000/MediaFileUpload', {
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        body: formData
      ...
      return (
        <div>
          ...
          {Feed.length === 0 ? '' :Feed.map((Data,i) => (
                <img width="140" height="140" src={Data.pix_loadPath!=undefined?'http://antsnest.co.kr:4000/ftp/MediaFile/'+Data.pix_loadPath:'http://antsnest.co.kr:4000/ftp/FileUpload/'+'NoImage.png'}/>
            ))}
            </div>
      </div>
      );
