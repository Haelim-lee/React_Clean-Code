...
function Infra(props) {
  ...
  const CreateSession=()=>{
    ...
    .then(res => res.json())
    .then((msg) => {
      if(msg.State ==='LOGIN') GetIfaPhoto_ORG();
      setSession(msg)})
  ...
  const uploadImgPreview=()=>{
    ...
    const GetIfaPhoto_ORG=()=>{
      fetch('http://antsnest.co.kr:4000/GetIfaPhoto_ORG',{credentials:'include'})
      .then(res => res.json())
    ...
    const uploadImage =()=> {
      ...
      fetch('http://antsnest.co.kr:4000/InfraFileUpload', {
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        body: formData
      ...
      return (
        <div>
          ...
          {Feed.length=== 0 ? '':Feed.map((Data,i) => (
                <img width="140" height="140" src={Data.pix_loadPath!=undefined?'http://antsnest.co.kr:4000/ftp/InfraFile/'+Data.pix_loadPath:'http://antsnest.co.kr:4000/ftp/FileUpload/'+'NoImage.png'}/>
            ))}
            </div>
      </div>
      );
