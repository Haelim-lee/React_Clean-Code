...
const CreateSession= async()=>{
  let SessionData = await new Promise(function(resolve) {
   ...
   const Search= async(Min,Max)=>{
    ...
    fetch('http://antsnest.co.kr:4000/SelectModelerBoardPage', {method: 'POST', mode: 'cors',  credentials:'include', headers: { 'Content-Type': 'application/json' } , body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param=>resolve(Param))
  ...
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectModelerBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param => resolve(Param))
...
//관리자용 데이터삭제 기능
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteModelerBoard', {
      method: 'POST',
      mode: 'cors',
 ...
 //컨텐츠 읽기
  const ReadContents = (Data)=>{
    fetch('http://antsnest.co.kr:4000/ReadModelerBoard', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
    ...
    const SearchCountPlus = (Data)=>{
      fetch('http://antsnest.co.kr:4000/ModelerSearchCountPlus', {
        method: 'POST',           
        mode: 'cors',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(Data)})
 ...
 const ReadComments = (Data) => {
  fetch('http://antsnest.co.kr:4000/ReadModelerComments', {
    method:'POST',
    mode: 'cors',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(Data)})
