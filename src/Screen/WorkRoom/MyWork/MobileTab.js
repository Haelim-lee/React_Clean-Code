...
import LinkModelerBoard from './Modeler/Mobile';
import LinkDesignBoard from './Designer/Mobile';
import LinkWorkBoard from './Work/Mobile';
import LinkPlanBoard from './Plan/Mobile';
...
return(
    <div style={TabPageStyle}>
        <ul style = {SubNavStyle} >
            ...
            <li style={ListStyle}><a id='Plan' className="" ref={Plan}   onClick={()=>ActiveCheck(Plan)}> 기획/설계</a></li>
            <li style={ListStyle}><a id='Work' className="" ref={Work}   onClick={()=>ActiveCheck(Work)}> 업무일지</a></li>
          </ul>
            <select style={{ width:"100%" ,textAlign: "center"}} value={SelecedCategory} defaultValue={"ALL"} type='text' onChange={SelectedCategory}>
            <option>ALL</option>
