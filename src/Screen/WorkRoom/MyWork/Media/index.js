...
//게시판속성
  ...
  const Search= async(Min,Max)=>{
    ...
    fetch('http://antsnest.co.kr:4000/SelectMediaBoardPage', {method: 'POST', mode: 'cors',   credentials:'include',headers: { 'Content-Type': 'application/json' } , body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param=>resolve(Param))
  ...
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectMediaBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param => resolve(Param))
 ...
 //관리자용 데이터삭제 기능
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteMediaBoard', {
      method: 'POST',
      mode: 'cors',
  ...
  const ReadContents = (Data)=>{
    fetch('http://antsnest.co.kr:4000/ReadMediaBoard', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
  ...
  const SearchCountPlus = (Data)=>{
    fetch('http://antsnest.co.kr:4000/MediaSearchCountPlus', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)})
      .catch(err=>alert('MediaSearchCountPlus'+ err));
  }

 // 댓글 가져오기
 const ReadComments = (Data) => {
  fetch('http://antsnest.co.kr:4000/ReadMediaComments', {
    method:'POST',
    mode: 'cors',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(Data)})
