...
function Contents_View(props){
  ...
  const [SendData,setSendData] = useState({"noticeNum":"", "id":"", "name":"", "commentText":""});
    ...
    const UpdateMode = ()=>{
        ReactDOM.render(<WriteMode Refrash={props.Refrash} Data = {props.Subject}/>,document.getElementById('Board_Contents'));
      }
    const DeleteItem=()=>{
    ...
    const InsertComment = (Data) => {
      fetch('http://antsnest.co.kr:4000/InsertDeveloperComment',{
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    const DeleteComment = (Data) => {
      fetch('http://antsnest.co.kr:4000/DeleteDeveloperComment',{
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        headers: { 'Content-Type': 'application/json' },
