...
//게시판속성
  ...
  const Search= async(Min,Max)=>{
    ...
    fetch('http://antsnest.co.kr:4000/SelectDeveloperBoardPage', {method: 'POST', mode: 'cors',  credentials:'include', headers: { 'Content-Type': 'application/json' } , body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param=>resolve(Param))
  ...
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectDeveloperBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param => resolve(Param))
    .catch(err=>alert('PageList 불러오기 메소드'+err));
      ...
      for(let 루프카운트=0; 루프카운트 < 정보[0].TOTALPAGE+1;루프카운트++){
        if(루프카운트+1>(현재페이지*10) && 루프카운트<((현재페이지+1)*10)) 
          페이지카운트.push(루프카운트+1);
      }
 ...
 //관리자용 데이터삭제 기능
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteDeveloperBoard', {
      method: 'POST',
      mode: 'cors',
  ...
  const ReadContents = (Data)=>{
    fetch('http://antsnest.co.kr:4000/ReadDeveloperBoard', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
  ...
  const SearchCountPlus = (Data)=>{
    fetch('http://antsnest.co.kr:4000/DeveloperSearchCountPlus', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)})
      .catch(err=>alert('DeveloperSearchCountPlus'+ err));
  }

 // 댓글 가져오기
 const ReadComments = (Data) => {
  fetch('http://antsnest.co.kr:4000/ReadDeveloperComments', {
    method:'POST',
    mode: 'cors',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(Data)})
