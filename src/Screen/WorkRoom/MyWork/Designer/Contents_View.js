...
function Contents_View(props){
  ...
  const [SendData,setSendData] = useState({"noticeNum":"", "id":"", "name":"", "commentText":""});
    ...
    const UpdateMode = ()=>{
        ReactDOM.render(<WriteMode ModeType={"UPDATE"} Refrash={props.Refrash} Data = {props.Subject}/>,document.getElementById('Board_Contents'));
      }
    const DeleteItem=()=>{
    ...
    const InsertComment = (Data) => {
      fetch('http://antsnest.co.kr:4000/InsertDesignComment',{
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    const DeleteComment = (Data) => {
      fetch('http://antsnest.co.kr:4000/DeleteDesignComment',{
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        headers: { 'Content-Type': 'application/json' },
