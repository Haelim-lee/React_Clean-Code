...
const CreateSession= async()=>{
  let SessionData = await new Promise(function(resolve) {
    ...
    const Search= async(Min,Max)=>{
      ...
      fetch('http://antsnest.co.kr:4000/SelectDesignBoardPage', {method: 'POST', mode: 'cors',  credentials:'include',headers: { 'Content-Type': 'application/json' } , body: JSON.stringify(Data)})
      .then((res)=>res.json())
      .then(Param=>resolve(Param))
  ...
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectDesignBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param => resolve(Param))
  ...
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteDesignBoard', {
      method: 'POST',
      mode: 'cors',
  ...
  const ReadContents = (Data)=>{
    fetch('http://antsnest.co.kr:4000/ReadDesignBoard', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
  ...
  const SearchCountPlus = (Data)=>{
    fetch('http://antsnest.co.kr:4000/DesignSearchCountPlus', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)})
      .catch(err=>alert('DesignSearchCountPlus'+ err));
  }

 // 댓글 가져오기
 const ReadComments = (Data) => {
  fetch('http://antsnest.co.kr:4000/ReadDesignComments', {
    method:'POST',
    mode: 'cors',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(Data)})
