...
function WorkBoard(props) {
  ...
  fetch('http://antsnest.co.kr:4000/SelectWorkBoardPage', {method: 'POST', mode: 'cors',  credentials:'include', headers: { 'Content-Type': 'application/json' } , body: JSON.stringify(Data)})
  .then((res)=>res.json())
  .then(Param=>resolve(Param))
...
//2.페이징카운트 기능
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectWorkBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Data)})
    .then((res)=>res.json())
    .then(Param => resolve(Param))
  ...
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteWorkBoard', {
      method: 'POST',
      mode: 'cors',
  ...
  const ReadContents = (Data)=>{
    fetch('http://antsnest.co.kr:4000/ReadWorkBoard', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
      ).then((res)=>res.json())
      .then((msg)=>ReadComments(msg[0]))//코멘트 불러오기
      .then(SearchCountPlus(Data))//카운트 증가
      .catch(err=>alert('Read'+ err));
  }
  // 조회수 증가
  const SearchCountPlus = (Data)=>{
    fetch('http://antsnest.co.kr:4000/WorkSearchCountPlus', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)})
      .catch(err=>alert('WorkSearchCountPlus'+ err));
  }
 // 댓글 가져오기
 const ReadComments = (Data) => {
  fetch('http://antsnest.co.kr:4000/ReadWorkComments', {
    method:'POST',
    mode: 'cors',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(Data)})
