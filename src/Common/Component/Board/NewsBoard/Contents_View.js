...
function Contents_View(props){
    ...
    const InsertComment = (Data) => {
      fetch('http://antsnest.co.kr:4000/InsertNewsComment',{
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    const DeleteComment = (Data) => {
      fetch('http://antsnest.co.kr:4000/DeleteNewsComment',{
        method: 'POST',
        mode: 'cors',
        credentials:'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div>
        <div className="ContentsBox">
            ...
            {/* {텍스트 Enert기능 추가} */}
            {/* {컨텐츠 !=undefined && typeof(컨텐츠.NOTICE_CONTENTS)==='string' ? (컨텐츠.NOTICE_CONTENTS).split('\n').map( line => (<span>{line}<br/></span>)) : ''} */}
