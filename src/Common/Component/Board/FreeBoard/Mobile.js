...
function FreeBoard(props) {
 ...
 // 액션
  const SelectFree=()=>{
    ...
    fetch('http://antsnest.co.kr:4000/SelectFree_ORG')
    .then(res => res.json())
    .then(msg =>set게시글(msg))
    .then(setSendData({"num":"", "id":"", "name":"", "title":""}))
    .catch(err=>alert('SelectFree'+err));
  }
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteFreeBoard', {
      ...
      ).then(()=>SelectFree()).catch(err=>alert('DataDelete'+err));
  }

  const DataUpdate = (SendData,Data)=>{
    var Send = {"NUM":Data.NUM, "NOTICE_TITLE":SendData.title, "NOTICE_CONTENTS": SendData.contents}
    fetch('http://antsnest.co.kr:4000/UpdateFreeBoard', {
      method: 'POST',
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Send)}
      ).then(()=>SelectFree()).catch(err=>alert('DataUpdate'+err));
  }
  ...
  if(Contents_Div.current != null)
  {
    if(Contents_Div.current.className =='Contents_Active'){
      if(현재컨텐츠.NUM == 컨텐츠.NUM){
        ...
        ReactDOM.render(<Contents_View Refrash={()=>SelectFree()}/>,document.getElementById('Board_Contents'));
      }
      else{
...
const ActionWriteMode=()=>{
  ...
  if(Contents_Div.current.className =='Contents_Deactive'){
      Contents_Div.current.className ='Contents_Active';
      ReactDOM.render(<WriteMode Refrash={()=>SelectFree()}/>,document.getElementById('Board_Contents'))
  }
  else{
      ReactDOM.render(<WriteMode Refrash={()=>SelectFree()}/>,document.getElementById('Board_Contents'))
  }
