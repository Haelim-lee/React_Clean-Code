...
function FreeBoard(props) {
  ...
  //액션
  const SelectFreeBoardPage=(Min,Max)=>{
  CurrentMinItem = Min;
  CurrentMaxItem = Max;
  ...
  fetch('http://antsnest.co.kr:4000/SelectFreeBoardPage', {
    method: 'POST',
    mode: 'cors',
  ...
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectFreeBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({"Data":"Data"})})
    .then((res)=>res.json())
  ...
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteFreeBoard', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
      ).then(()=>SelectFreeBoardPage(CurrentMinItem,CurrentMaxItem)).catch(err=>alert('DataDelete'+err));
  }

  const ReadContents = (Data)=>{
    fetch('http://antsnest.co.kr:4000/ReadFreeBoard', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
  ...
  const SearchCountPlus = (Data)=>{
    fetch('http://antsnest.co.kr:4000/FreeSearchCountPlus', {
      method: 'POST',           
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)})
      .catch(err=>alert('FreeSearchCountPlus'+ err));
  }

 // 댓글 가져오기
 const ReadComments = (Data) => {
  fetch('http://antsnest.co.kr:4000/ReadFreeComments', {
    method:'POST',
    mode: 'cors',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(Data)})
    ...
    if(Contents_Div.current!=null)
    {
      if(Contents_Div.current.className =='Contents_Active'){
        if(현재컨텐츠.NUM == 컨텐츠.NUM){
          ...
          ReactDOM.render(<Contents_View Refrash={()=>SelectFreeBoardPage(CurrentMinItem,CurrentMaxItem)}/>,document.getElementById('Board_Contents'));
        }
        else{
          ReactDOM.render(<Contents_View Subject={컨텐츠}
                                        CommentData={컨텐츠.CommentData} 
                                        CommentRefrash = {() => ReadContents(컨텐츠)} 
                                        Refrash={()=>SelectFreeBoardPage(CurrentMinItem,CurrentMaxItem)}/>
  ...
  const ActionWriteMode=()=>{
    ...
    if(Contents_Div.current.className =='Contents_Deactive'){
        Contents_Div.current.className ='Contents_Active';
        ReactDOM.render(<WriteMode Refrash={()=>SelectFreeBoardPage(CurrentMinItem,CurrentMaxItem)}/>,document.getElementById('Board_Contents'))
    }
    else{
        ReactDOM.render(<WriteMode Refrash={()=>SelectFreeBoardPage(CurrentMinItem,CurrentMaxItem)}/>,document.getElementById('Board_Contents'))
    }
  ...
  return (
    <div>
      <div ref={Contents_Div} id ='Board_Contents' className='Contents_Deactive'/> {/* 게시글 내용 나오는 부분 설정*/}
  ...
  <tr><th width='5%'> No </th><th width='70px'> 작성자 </th><th width='65%'> 내용 </th> <th width='70px'> 조회수 </th> {Session.UserId =='dexterity'?<th width='70px'> 삭제 </th>:''}</tr>
      ...
      {페이지리스트.map((Data,index)=>(<span>
        {(index == 0) && (Data > 1)? <a onClick={()=>prevPage()}> [이전]</a>:''}
          <a onClick={()=>SelectFreeBoardPage((Data)*10-9,(Data)*10)}>[{Data}]</a> 
