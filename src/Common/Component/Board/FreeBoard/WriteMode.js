...
function WriteMode(props){
    ...
    const Link =()=> {
      ...
      if(props.Data === undefined)
        fetch('http://antsnest.co.kr:4000/InsertFreeBoard', {
          method: 'POST',
          mode: 'cors',
    ...
    <div>
        <div>
          작성자 <input className='작성자_입력기' type='text' value={Session.UserId == undefined ? SendData.name: Session.Name}  onChange={(ev)=>setSendData({...SendData,name:ev.target.value})}></input><br/>
