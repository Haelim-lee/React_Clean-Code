...
function QaBoard(props) {
  ...
  const SelectQA=()=>{
    ...
    fetch('http://antsnest.co.kr:4000/SelectQA_ORG')
    .then(res => res.json())
    .then(msg =>set게시글(msg))
    .then(setSendData({"num":"", "id":"", "name":"", "title":""}))
    .catch(err=>alert('SelectQA'+err));
  }
  ...
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteQaBoard', {
      ...
      ).then(()=>SelectQA()).catch(err=>alert('DataDelete'+err));
  }
  ...
  const DataUpdate = (SendData,Data)=>{
    ...
    fetch('http://antsnest.co.kr:4000/UpdateQaBoard', {
      ...
      ).then(()=>SelectQA()).catch(err=>alert('DataUpdate'+err));
  }
  ...
  if(Contents_Div.current!=null)
  {
    if(Contents_Div.current.className =='Contents_Active'){
      if(현재컨텐츠.NUM == 컨텐츠.NUM){
        ...
        ReactDOM.render(<Contents_View Refrash={()=>SelectQA()}/>,document.getElementById('Board_Contents'));
      }
      else{
...
const ActionWriteMode=()=>{
  ...
  if(Contents_Div.current.className =='Contents_Deactive'){
      Contents_Div.current.className ='Contents_Active';
      ReactDOM.render(<WriteMode Refrash={()=>SelectQA()}/>,document.getElementById('Board_Contents'))
  }
  else{
      ReactDOM.render(<WriteMode Refrash={()=>SelectQA()}/>,document.getElementById('Board_Contents'))
  }
