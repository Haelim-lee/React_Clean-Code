...
function DeveloperNoteBoard(props) {
  ...
  //액션
  const SelectDeveloperNoteBoardPage=(Min,Max)=>{
  CurrentMinItem = Min;
  CurrentMaxItem = Max;
  ...
  fetch('http://antsnest.co.kr:4000/SelectDeveloperNoteBoardPage', {
    method: 'POST',
    mode: 'cors',
  ...
  const GetPageList= async () => {
    ...
    fetch('http://antsnest.co.kr:4000/SelectDeveloperNoteBoardPageList', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({"Data":"Data"})})
    .then((res)=>res.json())
  ...
  const DataDelete=(Data)=>{
    fetch('http://antsnest.co.kr:4000/DeleteDeveloperNoteBoard', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Data)}
      ).then(()=>SelectDeveloperNoteBoardPage(CurrentMinItem,CurrentMaxItem)).catch(err=>alert('DataDelete'+err));
  }
 ...
 const ReadComments = (Data) => {
    ...
    if(Contents_Div.current!=null)
    {
      if(Contents_Div.current.className =='Contents_Active'){
        if(현재컨텐츠.NUM == 컨텐츠.NUM){
          ...
          ReactDOM.render(<Contents_View Refrash={()=>SelectDeveloperNoteBoardPage(CurrentMinItem,CurrentMaxItem)}/>,document.getElementById('Board_Contents'));
        }
        else{
          ReactDOM.render(<Contents_View Subject={컨텐츠}
                                        CommentData={컨텐츠.CommentData} 
                                        CommentRefrash = {() => ReadContents(컨텐츠)} 
                                        Refrash={()=>SelectDeveloperNoteBoardPage(CurrentMinItem,CurrentMaxItem)}/>
  ...
  const ActionWriteMode=()=>{
    ...
    if(Contents_Div.current.className =='Contents_Deactive'){
        Contents_Div.current.className ='Contents_Active';
        ReactDOM.render(<WriteMode Refrash={()=>SelectDeveloperNoteBoardPage(CurrentMinItem,CurrentMaxItem)}/>,document.getElementById('Board_Contents'))
    }
    else{
        ReactDOM.render(<WriteMode Refrash={()=>SelectDeveloperNoteBoardPage(CurrentMinItem,CurrentMaxItem)}/>,document.getElementById('Board_Contents'))
    }
  ...
  // 화면로드
  useEffect(() => {
    SelectDeveloperNoteBoardPage('0','10');
    CreateSession();
  ...
  <tr><th width='5%'>No</th><th width='70px'>작성자</th><th width='65%'>내용</th> <th width='70px'>조회수</th> {Session.UserId =='dexterity'?<th width='70px'>삭제</th>:''}</tr>
      ...
      {페이지리스트.map((Data,index)=>(<span>
        {(index == 0) && (Data > 1)? <a onClick={()=>prevPage()}> [이전]</a>:''}
          <a onClick={()=>SelectDeveloperNoteBoardPage((Data)*10-9,(Data)*10)}>[{Data}]</a> 
