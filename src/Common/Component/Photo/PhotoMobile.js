...
function Photo(props) {
  ...
  const uploadImgPreview=()=>{
    ...
    reader.onload = function(){
      ...
      document.getElementById("thumbnailImg").width='200';
      document.getElementById("thumbnailImg").height='200';
    }
    if(fileinfo){
        ...
        <p align='center'>
          ...
          {Feed.map((Data,i) => (
            <div className='ReadModeBox' style={{borderRadius:'15px'}}>
              ...
              <div className='WriteModeHeader' style={{borderTopLeftRadius:'15px',borderTopRightRadius:'15px',borderRadius:'15px',backgroundColor:'whitesmoke'}}><img  style={{marginLeft:'10px',marginTop:'10px' }} className='프로필_이미지' src={'http://antsnest.co.kr:4000/ftp/FileUpload/' + profile.fileName}/>
              <label style={{marginLeft:'10px',lineHeight:'90px',fontWeight:'bold', fontSize:'20px'}}>{Session.UserId}</label></div>
        <div className='imgPixHeader'><img width="100%" src={Data.pix_loadPath!=undefined?'http://antsnest.co.kr:4000/ftp/FileUpload/'+Data.pix_loadPath:'NoImage.png'}/></div>
